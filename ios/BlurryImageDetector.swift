...
public class BlurryImageDetector: RCTEventEmitter {
  ...
  func getAssetThumbnail(asset: PHAsset) -> CGImage? {
    ...
    manager.requestImage(for: asset, targetSize: CGSize(width: 512, height: 512), contentMode: .aspectFit, options: option, resultHandler: { result, _in
  ...
  func getImageBlurriness(assetImage: CGImage) -> Double {
    ...
    let mtlDevice = mtlGlobalDevice!
    let mtlCommandQueue = mtlGlobalCommandQueue!
    ...
    do {
      ...
      let grayscaleTextureDescriptor = MTLTextureDescriptor.texture2DDescriptor(pixelFormat: pixelFormat, width: sourceTexture.width, height: sourceTexture.height, mipmapped: false)
      grayscaleTextureDescriptor.usage = [.shaderWrite, .shaderRead]

      guard let grayscaleTexture = mtlDevice.makeTexture(descriptor: grayscaleTextureDescriptor) else { return -1 }
      imageConversion.encode(commandBuffer: commandBuffer, sourceTexture: sourceTexture, destinationTexture: grayscaleTexture)

      // 2) Apply laplacian filter
      let laplacianTextureDescriptor = MTLTextureDescriptor.texture2DDescriptor(pixelFormat: pixelFormat, width: sourceTexture.width, height: sourceTexture.height, mipmapped: false)
      laplacianTextureDescriptor.usage = [.shaderWrite, .shaderRead]

      guard let laplacianTexture = mtlDevice.makeTexture(descriptor: laplacianTextureDescriptor) else { return -1 }
      laplacian.encode(commandBuffer: commandBuffer, sourceTexture: grayscaleTexture, destinationTexture: laplacianTexture)
      ...
      let bytes = getTextureBytes(texture: laplacianTexture).map { Double($0) }
  ...
  public func fetchImagesFromGallery(threshold: Double, onProgress: @escaping (PHAsset) -> Void, onFinished: @escaping () -> Void) {
    PHPhotoLibrary.requestAuthorization { status in
      ...
      case .authorized:
        ...
        let count: Int = assets.count < 500 ? assets.count : 500
        ...
        for i in 0 ..< count {
          autoreleasepool {
            ...
            let percent = Int(i * 100 / count)
            if percent != prevPercent {
              self.sendEvent(withName: "OnProgress", body: [
                "percent": percent,
              ])
  ...
  @objc(findBlurryImagesFromGallery:withThreshold:withResolver:withRejecter:)
  func findBlurryImagesFromGallery(previousIds: [String], threshold: Double, resolve: RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
    fetchImagesFromGallery(threshold: threshold,
                           onProgress: {
                             (asset: PHAsset) in
                             let resources = PHAssetResource.assetResources(for: asset)
                             let fileSize = resources
                               .map { $0.value(forKey: "fileSize") as? Int64 ?? 0 }
                               .reduce(0) { acc, item in acc + item }

                             self.sendEvent(withName: "OnImageFound", body: [
                               "id": asset.localIdentifier,
                               "size": fileSize,
                               "date": asset.creationDate?.timeIntervalSince1970 ?? 0,
                               "filename": resources.count > 0 ? resources[0].originalFilename : "",
                             ])
                           }, onFinished: {
                             () in
                             self.sendEvent(withName: "OnFinished", body: nil)
                           })
  }
}
